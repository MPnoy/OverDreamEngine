...
namespace ODEngine.Game.Scenario
{
    public class ScenarioManager
    {
        ...
        //пути
        public readonly string pathCompList = "Text/LinkList.txt";
        public readonly string pathCharList = "Text/ChList.txt";
        public readonly string pathScenario = "Text/Scenario";
        ...
        public void Init()
        {
            ...
            if (FileManager.SystemExists("cache.tmp"))
            {
                try
                {
                    resourceCache = JsonSerializer.Deserialize<ResourceCache>(FileManager.SystemReadAllText("cache.tmp", System.Text.Encoding.Unicode));
                }
                catch
                {
                    FileManager.Delete("cache.tmp");
        ...
        public void CompInterpretation(string fileName) // Обработка композиций
        {
            ...
            var TxtList = FileManager.DataReadAllText(fileName).Split(new string[] { "\r\n" }, StringSplitOptions.None).ToList();
                    ...
                    case "$anim": // Покадровая анимация
                        {
                            ...
                            if (ar.Length == 4)
                            {
                                ...
                                var paths = FileManager.DataGetFiles(ar[2], "*.png");
                            ...
                            else
                            {
                                ...
                                for (int j = 2; j < ar.Length; j += 2)
                                {
                                    ar[j] = ReplaceLinks(compositionsStr, ar[j]);
                                    list.Add(new ImageCompositionFrameAnimation.FrameItemPrototype(ImageCompositionStaticSimplex.GetComposition(ar[j] + ".png", resourceCache), float.Parse(ar[j + 1])));
                                }
                            }
        ...
        {
            var filenames = FileManager.DataGetFiles(scenarioFolder, "*.txt", System.IO.SearchOption.AllDirectories);
            var ar = new string[filenames.Length][];

            for (int i = 0; i < filenames.Length; i++)
            {
                ar[i] = FileManager.DataReadAllLines(filenames[i]);
            }
            ...
            // Сохранение кэша
            FileManager.WriteAllText("cache.tmp", JsonSerializer.Serialize(resourceCache, new JsonSerializerOptions() { WriteIndented = true }), System.Text.Encoding.Unicode);
