...
namespace ODEngine.EC.Components
{
    public class GUIElement : Component
    {
        ...
        public event EventHandler<(Vector2 mousePosition, MouseButton mouseButton)> GlobalMouseUp;
        public event EventHandler<Vector2> GlobalMouseMove;
        ...
        {
            GlobalMouseMove?.Invoke(this, mousePosition);

            foreach (var item in Input.mouseUps)
            {
                GlobalMouseUp?.Invoke(this, (mousePosition, item));
            }
        ...
        {
            return CreateContainer(parent, position, size, new Material(null, fragShader), true);
        }
        ...
        public static GUIElement CreateFrameAnimation(Renderer parent, Vector3 position, Vector2 size, params (string imageFile, ColorMatrix colorMatrix, float time)[] images)
        {
            ...
            for (int i = 0; i < images.Length; i++)
            {
                ...
                {
                    var imageFile = "Images/" + image.imageFile + ".png";
