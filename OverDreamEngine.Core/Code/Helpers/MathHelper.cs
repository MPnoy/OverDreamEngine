...
namespace ODEngine.Helpers
{
    ...
    {
        public static readonly Vector4 oneRect = new Vector4(0f, 0f, 1f, 1f);
        ...
        public static Vector4Int GetGlobalRect(Vector2Int spriteSizePixels, float scale, float x, float y) // Получить прямоугольник обрезки спрайта экраном в пикселях в координатах экрана
        {
            ...
            return new Vector4Int(
                RoundToInt(MathF.Max(x * Graphics.cameraMultiplier - scaled.X / 2f, -Graphics.cameraWidth / 2f)),
                RoundToInt(MathF.Max(y * Graphics.cameraMultiplier - scaled.Y / 2f, -Graphics.cameraHeight / 2f)),
                RoundToInt(MathF.Min(x * Graphics.cameraMultiplier + scaled.X / 2f, Graphics.cameraWidth / 2f)),
                RoundToInt(MathF.Min(y * Graphics.cameraMultiplier + scaled.Y / 2f, Graphics.cameraHeight / 2f)));
        }
        ...
        public static Vector4Int GetRect(Vector2Int spriteSizePixels, float scale, float x, float y)
        {
            ...
            var screenRelativeSprite = new Vector4Int(
                RoundToInt(-Graphics.cameraWidth / 2f - x * Graphics.cameraMultiplier),
                RoundToInt(-Graphics.cameraHeight / 2f - y * Graphics.cameraMultiplier),
                RoundToInt(Graphics.cameraWidth / 2f - x * Graphics.cameraMultiplier),
                RoundToInt(Graphics.cameraHeight / 2f - y * Graphics.cameraMultiplier));
            return new Vector4Int(
        ...
        public static Vector2 Vec2DivVec2(Vector2 vec1, Vector2 vec2)
        {
            return new Vector2(
                vec1.X / vec2.X,
                vec1.Y / vec2.Y);
        ...
        {
            return GetRectInNewBasis(normalized, oneRect);
        }
