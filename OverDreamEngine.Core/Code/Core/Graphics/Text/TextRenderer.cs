...
namespace ODEngine.Core.Text
{
    public class TextRenderer : IDisposable
    {
        ...
        public TextRenderer()
        {
            material = new Material("Text", "Text")
            {
            ...
            var files = FileManager.DataGetFiles("Fonts", "*.*", System.IO.SearchOption.AllDirectories);

            for (int i = 0; i < files.Length; i++)
            {
                for (int j = 8; j <= 64; j *= 2)
                {
                    atlases.Add((FileManager.GetFileNameWithoutExtension(files[i]).ToLower(), j),
                        new Atlas(new Face(library, FileManager.DataReadAllBytes(files[i]), 0), j));
                }
            }
        }
        ...
        public float RenderText(TextColored text, string font, RenderTexture texture, Vector2 boxSize, float charHeight, float interval, bool onlyReturnHeight = false, TextAlign textAlign = TextAlign.Left)
        {
            ...
            int charHeightPixel2 = Math.Clamp((int)Math.Pow(2, (int)MathF.Log2(charHeightPixel) + 1), 8, 64);
