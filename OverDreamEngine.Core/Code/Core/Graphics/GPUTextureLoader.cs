...
namespace ODEngine.Core
{
    public static class GPUTextureLoader
    {
        ...
        public static RenderTexture LoadSync(string filename)
        {
            var rawImage = LoadRawSync(filename);
        ...
        public static Ticket LoadAsync(string filename, SixLabors.ImageSharp.ColorMatrix colorMatrix)
        {
            ...
            IEnumerator routine()
            {
                var imageTicket = LoadRaw(filename, colorMatrix, 1f / Kernel.settings.TextureSizeDiv);
        ...
        public static Ticket LoadAsync(string filename, int width, int height)
        {
            ...
            IEnumerator routine()
            {
                var texture = RenderTexture.GetTemporary(width / Kernel.settings.TextureSizeDiv, height / Kernel.settings.TextureSizeDiv);

                var imageTicket = LoadRaw(new[] { filename }, new Vector2Int(texture.Width / Kernel.settings.TextureSizeDiv, texture.Height / Kernel.settings.TextureSizeDiv), SixLabors.ImageSharp.ColorMatrix.Identity);
                while (!imageTicket.isLoaded)
                {
                    if (ticket.isCanceled)
                    {
        ...
        public static void Init()
        {
            ...
            maxTextureSize = 8192 / Kernel.settings.TextureSizeDiv * 4096 / Kernel.settings.TextureSizeDiv * 4;
