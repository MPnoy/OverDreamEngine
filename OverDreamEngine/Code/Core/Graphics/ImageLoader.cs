    ...
    public class ImageLoader
    {
        public class Processor
        {
            ...
            private void ProcessRequest(Request request)
            {
                ...
                //Загрузка
                var loadingTasks = new Task<Image>[request.paths.Length];
                for (int i = 0; i < loadingTasks.Length; i++)
                {
#if DEBUG
                    if (!File.Exists(request.paths[i]))
                    {
                        Debug.Print("Invalid path: " + request.paths[i]);
                        return;
                    }
#endif
                    loadingTasks[i] = Image.LoadAsync(request.paths[i], ct);
                }
        ...
        public struct Request
        {
            ...
            public float scale;
            public SixLabors.ImageSharp.ColorMatrix colorMatrix;
        }
