...
namespace ODEngine.EC.Components
{
    public class GameText : Component
    {
        ...
        internal float maskTop;
        internal Size maskSize;
        internal int nowLine = 0;
        internal float maskXProgress;
        internal List<Point> lineEnds;
        internal int renderHeight;

        private float lineInterval = 0.06f;
        private float lineIntervalOffset = 0f;

        internal IEnumerator maskCoroutine = null;
        ...
        protected override void OnCreate()
        {
            ...
            textBox.Text = "";
            offsetMaterial = new Material("Game/Offset", "Game/Offset", "Game/ColorMul")
            {
            ...
            if (outline)
            {
                material = new Material("Game/TextMaskDiv", null, "Game/TextMaskDiv");
            }
            else
            {
                material = new Material("Game/TextMask", null, "Game/TextMask");
            }
            UpdateSize(1f, 1f);
        ...
        internal void MaskEnd(bool setMaskStart = false)
        {
            MaskStop();
        ...
        internal void MaskResume(float step, Action onDone = null)
        {
            IEnumerator routine()
            {
                while (!MaskAdd(step * Kernel.deltaTimeUpdate))
        ...
        internal bool IsTruncated(string testText)
        {
