...
public static class MaterialReader
{
    ...
    {
        string text = File.ReadAllText(PathBuilder.dataPath + "Text/Materials.json");
        materialsInfo = JsonSerializer.Deserialize<MaterialsInfo>(text);
        materials = new Dictionary<string, Material>();
        foreach (var matInfo in materialsInfo.Materials)
        {
            var mat = new Material("Game/" + matInfo.Shader, "Atlas/Identity", "Game/" + matInfo.Shader);
            foreach (var variable in matInfo.Variables)
            {
                switch (variable.Type)
                {
                    case "Texture":
                        {
                            using var image = Image.Load<Rgba32>(PathBuilder.dataPath + variable.Value + ".png");
